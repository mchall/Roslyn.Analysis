...
using Portal.CodeAnalyzer.StandAlone.Modules;

namespace Portal.CodeAnalyzer.StandAlone
{
    internal class Program
    {
        ...
        private static async void DoCodeRefactor()
        {
            ...
            var solution = await workspace.OpenSolutionAsync(@"C:\Working\portal\Property24.sln");

            if (solution != null)
            {
                //Constrain project here
                foreach (var project in solution.Projects.Where(p => p.Name == "Portal.Domain"))
                {
